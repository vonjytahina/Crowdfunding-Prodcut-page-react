...
function Header() {
  ...
  useEffect(() => {
    const mobileMenus = mobileMenu.current;
    const hamburgerButtons = hamurgerButton.current;
    ...
    const windowResize = () => {
      if (document.documentElement.clientWidth > 376) {
        ...
        setVignetteVisibility(false);
        if (mobileMenus) {
          mobileMenus.classList.remove("show-mobile-menu");
          mobileMenus.classList.remove("hide-mobile-menu");
        }
        if (hamburgerButtons) {
          hamburgerButtons.classList.add("hidden");
        }
      } else if (document.documentElement.clientWidth <= 375) {
        mobileMenus.classList.remove("show-mobile-menu");
        mobileMenus.classList.remove("hide-mobile-menu");
        hamburgerButtons.classList.remove("hidden");
      }
    };
    ...
    <header>
      ...
      <div className="right">
        ...
        <svg
          ...
          ref={hamurgerButton}
          style={
            !isMobileMenuVisible ? { display: "block" } : { display: "none" }
          }
          width="16"
          height="15"
      ...
      <div
        className={`mobile-menu ${
          isMobileMenuVisible ? "show-mobile-menu" : "hide-mobile-menu"
        }`}
        ref={mobileMenu}
      >
        <ul>
